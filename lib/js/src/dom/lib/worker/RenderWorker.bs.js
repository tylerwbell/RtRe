...
function render(scene, command) {
  ...
  for(var dx = 0 ,dx_finish = Rect$Rt.width(frame); dx < dx_finish; ++dx){
    for(var dy = 0 ,dy_finish = Rect$Rt.height(frame); dy < dy_finish; ++dy){
      ...
      var color = Tracer$Rt.trace(scene, ray, 5);
      Curry._4(Collections$Rt.Array2d.set, slice.buffer, dx, dy, color);
      Curry._4(Collections$Rt.Array2d.set, slice.samples, dx, dy, 1);
    }
  }
  return slice;
}
