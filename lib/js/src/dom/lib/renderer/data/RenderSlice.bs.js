...
function make(frame, clearColor) {
  ...
  var samples = Curry._3(Collections$Rt.Array2d.make, frame.size.width, frame.size.height, 0);
  return {
          frame: frame,
          buffer: buffer,
          samples: samples
...
function clear(t, clearColor) {
  ...
  return Curry._2(Collections$Rt.Array2d.fill, t.samples, 0);
}
...
function blend(dest, source) {
  for(var sourceX = 0 ,sourceX_finish = source.frame.size.width; sourceX < sourceX_finish; ++sourceX){
    for(var sourceY = 0 ,sourceY_finish = source.frame.size.height; sourceY < sourceY_finish; ++sourceY){
      ...
      var destSamples = Curry._3(Collections$Rt.Array2d.get, dest.samples, destX, destY);
      var sourceSamples = Curry._3(Collections$Rt.Array2d.get, source.samples, sourceX, sourceY);
      ...
      Curry._4(Collections$Rt.Array2d.set, dest.samples, destX, destY, destSamples + sourceSamples | 0);
    }
  }
