...
let multScalar = (v: t, scalar: float) => {
  x: scalar *. v.x,
  y: scalar *. v.y,
  z: scalar *. v.z,
};
let divScalar = (v: t, scalar: float) => multScalar(v, 1.0 /. scalar);
...
let mult = (a: t, b: t) => {x: a.x *. b.x, y: a.y *. b.y, z: a.z *. b.z};
...
let normalized = (v: t) => divScalar(v, length(v));